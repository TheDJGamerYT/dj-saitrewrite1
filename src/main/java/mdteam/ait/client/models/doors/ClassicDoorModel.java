...
public class ClassicDoorModel extends DoorModel {
	...
	public void renderWithAnimations(DoorBlockEntity doorEntity, ModelPart root, MatrixStack matrices, VertexConsumer vertices, int light, int overlay, float red, float green, float blue, float pAlpha) {
		...
		this.classic.getChild("Doors").getChild("left_door").yaw = (doorEntity.getClientTardis().getExterior().isLeftDoorOpen() || doorEntity.getClientTardis().getExterior().isBothDoorsOpen())  ? -5F : 0.0F;
		this.classic.getChild("Doors").getChild("right_door").yaw = (doorEntity.getClientTardis().getExterior().isRightDoorOpen() || doorEntity.getClientTardis().getExterior().isBothDoorsOpen()) ? 5F : 0.0F;
